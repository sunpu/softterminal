...
SwitchButton::SwitchButton(QWidget *parent) : QWidget(parent)
{
	...
	bgColorOff = QColor(145, 146, 155);
	bgColorOn = QColor(48, 186, 120);
	...
	textOff = QStringLiteral("已关闭");
	textOn = QStringLiteral("已开启");
...
void SwitchButton::drawText(QPainter *painter)
{
	...
	if (!checked)
	{
		...
		painter->drawText(height(), 0, width() - height(), height(), Qt::AlignCenter, textOff);
	}
	else
	{
		painter->setPen(textColorOn);
		painter->drawText(space, 0, width() - height(), height(), Qt::AlignCenter, textOn);
	}
